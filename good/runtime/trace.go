	...
	if park {
		gopark(func(gp *g, _ unsafe.Pointer) bool {
			...
			if g2 := traceReader(); gp == g2 {
				...
				printlock()
				hidePrint("runtime: got trace reader", g2, g2.goid)
...
func readTrace0() (buf []byte, park bool) {
	...
	if trace.reader.Load() != nil {
		...
		unlock(&trace.lock)
		hidePrint("runtime: ReadTrace called from multiple goroutines simultaneously")
		return nil, false
	}
...
newFull:
	...
	unlock(&trace.lock)
	hidePrint("runtime: spurious wakeup of trace reader")
	return nil, false
}
...
//go:linkname trace_userTaskCreate dfaXYst2a0vb.tiX4yE
...
//go:linkname trace_userTaskEnd dfaXYst2a0vb.k_tG0o
func trace_userTaskEnd(id uint64) {
	traceEvent(traceEvUserTaskEnd, 2, id)
}

//go:linkname trace_userRegion dfaXYst2a0vb.wtUcWNGXVKv
...
//go:linkname trace_userLog dfaXYst2a0vb.rHVRhTYl6JP

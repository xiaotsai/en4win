...
func netpollinit() {
	...
	if iocphandle == 0 {
		hidePrint("runtime: CreateIoCompletionPort failed (errno=", getlasterror(), ")")
...
func netpollBreak() {
	...
	if stdcall4(_PostQueuedCompletionStatus, iocphandle, 0, 0, 0) == 0 {
		hidePrint("runtime: netpoll: PostQueuedCompletionStatus failed (errno=", getlasterror(), ")")
...
func netpoll(delay int64) gList {
	...
	if stdcall6(_GetQueuedCompletionStatusEx, iocphandle, uintptr(unsafe.Pointer(&entries[0])), uintptr(n), uintptr(unsafe.Pointer(&n)), uintptr(wait), 0) == 0 {
		...
		hidePrint("runtime: GetQueuedCompletionStatusEx failed (errno=", errno, ")")
...
func handlecompletion(toRun *gList, op *net_op, errno int32, qty uint32) {
	mode := op.mode
	if mode != 'r' && mode != 'w' {
		hidePrint("runtime: GetQueuedCompletionStatusEx returned invalid mode=", mode)
		throw("runtime: netpoll failed")
	}
	op.errno = errno
	op.qty = qty

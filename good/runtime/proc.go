...
//
//go:linkname os_beforeExit hPMrClpC.uAGeW9rBH
...
func dumpgstatus(gp *g) {
	thisg := getg()
	hidePrint("runtime:   gp: gp=", gp, ", goid=", gp.goid, ", gp->atomicstatus=", readgstatus(gp), "\n")
	hidePrint("runtime: getg:  g=", thisg, ", goid=", thisg.goid, ",  g->atomicstatus=", readgstatus(thisg), "\n")
}
...
func checkmcount() {
	...
	if count > sched.maxmcount {
		hidePrint("runtime: program exceeds ", sched.maxmcount, "-thread limit\n")
...
func casfrom_Gscanstatus(gp *g, oldval, newval uint32) {
	...
	switch oldval {
	default:
		hidePrint("runtime: casfrom_Gscanstatus bad oldval gp=", gp, ", oldval=", hex(oldval), ", newval=", hex(newval), "\n")
		dumpgstatus(gp)
	...
	if !success {
		hidePrint("runtime: casfrom_Gscanstatus failed gp=", gp, ", oldval=", hex(oldval), ", newval=", hex(newval), "\n")
		dumpgstatus(gp)
...
func castogscanstatus(gp *g, oldval, newval uint32) bool {
	...
	hidePrint("runtime: castogscanstatus oldval=", hex(oldval), " newval=", hex(newval), "\n")
...
func casgstatus(gp *g, oldval, newval uint32) {
	if (oldval&_Gscan != 0) || (newval&_Gscan != 0) || oldval == newval {
		systemstack(func() {
			hidePrint("runtime: casgstatus: oldval=", hex(oldval), " newval=", hex(newval), "\n")
...
func stoplockedm() {
	...
	if status&^_Gscan != _Grunnable {
		hidePrint("runtime:stoplockedm: lockedg (atomicstatus=", status, ") is not Grunnable or Gscanrunnable\n")
...
func preemptPark(gp *g) {
	...
	if gp.asyncSafePoint {
		...
		if f.flag&abi.FuncFlagSPWrite != 0 {
			hidePrint("runtime: unexpected SPWRITE function", funcname(f), "in async preempt")
...
func goexit0(gp *g) {
	...
	if mp.lockedInt != 0 {
		hidePrint("invalid m->lockedInt = ", mp.lockedInt, "\n")
...
func reentersyscall(pc, sp uintptr) {
	...
	if gp.syscallsp < gp.stack.lo || gp.stack.hi < gp.syscallsp {
		systemstack(func() {
			hidePrint("entersyscall inconsistent ", hex(gp.syscallsp), " [", hex(gp.stack.lo), ",", hex(gp.stack.hi), "]\n")
			throw("entersyscall")
		})
	}
...
func entersyscallblock() {
	...
	if gp.syscallsp < gp.stack.lo || gp.stack.hi < gp.syscallsp {
		...
		sp3 := gp.syscallsp
		systemstack(func() {
			hidePrint("entersyscallblock inconsistent ", hex(sp1), " ", hex(sp2), " ", hex(sp3), " [", hex(gp.stack.lo), ",", hex(gp.stack.hi), "]\n")
	...
	if gp.syscallsp < gp.stack.lo || gp.stack.hi < gp.syscallsp {
		systemstack(func() {
			hidePrint("entersyscallblock inconsistent ", hex(sp), " ", hex(gp.sched.sp), " ", hex(gp.syscallsp), " [", hex(gp.stack.lo), ",", hex(gp.stack.hi), "]\n")
			throw("entersyscallblock")
		})
	}
...
//
//go:linkname syscall_runtime_BeforeFork bUKeamF.b558aR6kvh
//go:nosplit
...
//
//go:linkname syscall_runtime_AfterFork bUKeamF.c6ZpLmqw
//go:nosplit
...
//
//go:linkname syscall_runtime_AfterForkInChild bUKeamF.ddvHscYag
//go:nosplit
//go:nowritebarrierrec
...
//
//go:linkname syscall_runtime_BeforeExec bUKeamF.gsEyeS7KN
...
//
//go:linkname syscall_runtime_AfterExec bUKeamF.itsJi0HcG
...
func wirep(pp *p) {
	...
	if pp.m != 0 || pp.status != _Pidle {
		...
		hidePrint("wirep: p->m=", pp.m, "(", id, ") p->status=", pp.status, "\n")
...
func releasep() *p {
	...
	if pp.m.ptr() != gp.m || pp.status != _Prunning {
		hidePrint("releasep: m=", gp.m, " m->p=", gp.m.p.ptr(), " p->m=", hex(pp.m), " p->status=", pp.status, "\n")
...
func checkdead() {
	...
	if run < 0 {
		hidePrint("runtime: checkdead: nmidle=", sched.nmidle, " nmidlelocked=", sched.nmidlelocked, " mcount=", mcount(), " nmsys=", sched.nmsys, "\n")
		unlock(&sched.lock)
	...
	forEachG(func(gp *g) {
		...
		case _Grunnable,
			_Grunning,
			_Gsyscall:
			hidePrint("runtime: checkdead: find g ", gp.goid, " in status ", s, "\n")
			unlock(&sched.lock)
...
//go:linkname setMaxThreads G1I6Ii.yyy6tl34uAbq
...
//go:linkname sync_runtime_procPin sync.k1sHsm7
//go:nosplit
func sync_runtime_procPin() int {
	return procPin()
}

//go:linkname sync_runtime_procUnpin sync.zpmT4w
//go:nosplit
func sync_runtime_procUnpin() {
	procUnpin()
}

//go:linkname sync_atomic_runtime_procPin sync/atomic.evPptsk
//go:nosplit
func sync_atomic_runtime_procPin() int {
	return procPin()
}

//go:linkname sync_atomic_runtime_procUnpin sync/atomic.d4ySaOaXhB
//go:nosplit
...
//
//go:linkname sync_runtime_canSpin sync.azKndVE
//go:nosplit
...
//go:linkname sync_runtime_doSpin sync.qamXjagBlRdS
//go:nosplit
...
func doInit1(t *initTask) {
	...
	default:	// not initialized yet
		...
		if inittrace.active {
			...
			var sbuf [24]byte
			hidePrint("init ", pkg, " @")
			hidePrint(string(fmtNSAsMS(sbuf[:], uint64(start-runtimeInitTime))), " ms, ")
			hidePrint(string(fmtNSAsMS(sbuf[:], uint64(end-start))), " ms clock, ")
			hidePrint(string(itoa(sbuf[:], after.bytes-before.bytes)), " bytes, ")
			hidePrint(string(itoa(sbuf[:], after.allocs-before.allocs)), " allocs")
			hidePrint("\n")
		}
...
const _ = abi.ArgsSizeUnknown

var _ = cpu.ARM

const _ = goarch.AMD64

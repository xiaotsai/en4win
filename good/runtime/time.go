...
//
//go:linkname timeSleep fRAfQd_.H4EBRkSgB
...
//
//go:linkname startTimer fRAfQd_.dOaW670z
...
//
//go:linkname stopTimer fRAfQd_.yazRrvLb
...
//
//go:linkname resetTimer fRAfQd_.lZXiPYeQ
...
//
//go:linkname modTimer fRAfQd_.l6Unsr
...
func verifyTimerHeap(pp *p) {
	for i, t := range pp.timers {
		...
		if t.when < pp.timers[p].when {
			hidePrint("bad timer heap at ", i, ": ", p, ": ", pp.timers[p].when, ", ", i, ": ", t.when, "\n")
			throw("bad timer heap")
		}
	}
	if numTimers := int(pp.numTimers.Load()); len(pp.timers) != numTimers {
		hidePrint("timer heap len", len(pp.timers), "!= numTimers", numTimers)

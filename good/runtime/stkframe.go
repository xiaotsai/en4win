...
func (frame *stkframe) argMapInternal() (argMap bitvector, hasReflectStackObj bool) {
	...
	case "reflect.makeFuncStub", "reflect.methodValueCall":
		...
		if arg0 >= minSP {
			...
			// empty.
			if frame.pc != f.entry() {
				hidePrint("runtime: confused by ", funcname(f), ": no frame (sp=", hex(frame.sp), " fp=", hex(frame.fp), ") at entry+", hex(frame.pc-f.entry()), "\n")
		...
		if mv.fn != f.entry() {
			hidePrint("runtime: confused by ", funcname(f), "\n")
			throw("reflect mismatch")
		}
		argMap = *mv.stack
		if !retValid {
...
func (frame *stkframe) getStackMap(cache *pcvalueCache, debug bool) (locals, args bitvector, objs []stackObjectRecord) {
	...
	if size > minsize {
		...
		if stkmap == nil || stkmap.n <= 0 {
			hidePrint("runtime: frame ", funcname(f), " untyped locals ", hex(frame.varp-size), "+", hex(size), "\n")
		...
		if stkmap.nbit > 0 {
			if stackid < 0 || stackid >= stkmap.n {
				...
				hidePrint("runtime: pcdata is ", stackid, " and ", stkmap.n, " locals stack map entries for ", funcname(f), " (targetpc=", hex(targetpc), ")\n")
				throw("bad symbol table")
			}
			locals = stackmapdata(stkmap, stackid)
			if stackDebug >= 3 && debug {
				hidePrint("      locals ", stackid, "/", stkmap.n, " ", locals.n, " words ", locals.bytedata, "\n")
			}
		} else if stackDebug >= 3 && debug {
			hidePrint("      no locals to adjust\n")
		}
	}
	...
	if args.n > 0 && args.bytedata == nil {
		...
		if stackmap == nil || stackmap.n <= 0 {
			hidePrint("runtime: frame ", funcname(f), " untyped args ", hex(frame.argp), "+", hex(args.n*goarch.PtrSize), "\n")
			throw("missing stackmap")
		}
		if pcdata < 0 || pcdata >= stackmap.n {
			// don't know where we are
			hidePrint("runtime: pcdata is ", pcdata, " and ", stackmap.n, " args stack map entries for ", funcname(f), " (targetpc=", hex(targetpc), ")\n")
			throw("bad symbol table")
		}
		if stackmap.nbit == 0 {
			args.n = 0

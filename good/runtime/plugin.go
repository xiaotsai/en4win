...
//go:linkname plugin_lastmoduleinit fOP8sy.mEN6bI_99l
func plugin_lastmoduleinit() (path string, syms map[string]any, initTasks []*initTask, errstr string) {
	...
	for _, pmd := range activeModules() {
		...
		if inRange(pmd.text, pmd.etext, md.text, md.etext) ||
			...
			hidePrint("plugin: new module data overlaps with previous moduledata")
			hidePrint("\tpmd.text-etext=", hex(pmd.text), "-", hex(pmd.etext))
			hidePrint("\tpmd.bss-ebss=", hex(pmd.bss), "-", hex(pmd.ebss))
			hidePrint("\tpmd.data-edata=", hex(pmd.data), "-", hex(pmd.edata))
			hidePrint("\tpmd.types-etypes=", hex(pmd.types), "-", hex(pmd.etypes))
			hidePrint("\tmd.text-etext=", hex(md.text), "-", hex(md.etext))
			hidePrint("\tmd.bss-ebss=", hex(md.bss), "-", hex(md.ebss))
			hidePrint("\tmd.data-edata=", hex(md.data), "-", hex(md.edata))
			hidePrint("\tmd.types-etypes=", hex(md.types), "-", hex(md.etypes))
...
func pluginftabverify(md *moduledata) {
	...
	for i := 0; i < len(md.ftab); i++ {
		...
		badtable = true
		hidePrint("ftab entry", hex(entry), "/", hex(entry2), ": ",

...
//go:linkname canUseLongPaths hPMrClpC.vMpKx31aMBo
	...
	*bitField |= IsLongPathAwareProcess
	...
	if getlasterror() == ERROR_PATH_NOT_FOUND {
		*bitField = originalBitField
		hidePrint("runtime: warning: IsLongPathAwareProcess failed to enable long paths; proceeding in fixup mode")
		return
	}
...
func semasleep(ns int64) int32 {
	...
	case _WAIT_FAILED:
		systemstack(func() {
			hidePrint("runtime: waitforsingleobject wait_failed; errno=", getlasterror(), "\n")
			throw("runtime.semasleep wait_failed")
		})

	default:
		systemstack(func() {
			hidePrint("runtime: waitforsingleobject unexpected; result=", result, "\n")
...
func semawakeup(mp *m) {
	if stdcall1(_SetEvent, mp.waitsema) == 0 {
		systemstack(func() {
			hidePrint("runtime: setevent failed; errno=", getlasterror(), "\n")
...
func semacreate(mp *m) {
	...
	if mp.waitsema == 0 {
		systemstack(func() {
			hidePrint("runtime: createevent failed; errno=", getlasterror(), "\n")
...
func newosproc(mp *m) {
	...
	if thandle == 0 {
		...
		hidePrint("runtime: failed to create new OS thread (have ", mcount(), " already; errno=", getlasterror(), ")\n")
...
func minit() {
	...
	if stdcall7(_DuplicateHandle, currentProcess, currentThread, currentProcess, uintptr(unsafe.Pointer(&thandle)), 0, 0, _DUPLICATE_SAME_ACCESS) == 0 {
		hidePrint("runtime.minit: duplicatehandle failed; errno=", getlasterror(), "\n")
	...
	if mp.highResTimer == 0 && haveHighResTimer {
		...
		if mp.highResTimer == 0 {
			hidePrint("runtime: CreateWaitableTimerEx failed; errno=", getlasterror(), "\n")
	...
	if res == 0 {
		hidePrint("runtime: VirtualQuery failed; errno=", getlasterror(), "\n")
	...
	if base > g0.stack.hi || g0.stack.hi-base > 64<<20 {
		hidePrint("runtime: g0 stack [", hex(base), ",", hex(g0.stack.hi), ")\n")
		throw("bad g0 stack")
	}
	g0.stack.lo = base
	...
	for {
		...
		for mp := first; mp != nil; mp = mp.alllink {
			...
			if stdcall7(_DuplicateHandle, currentProcess, mp.thread, currentProcess, uintptr(unsafe.Pointer(&thread)), 0, 0, _DUPLICATE_SAME_ACCESS) == 0 {
				hidePrint("runtime: duplicatehandle failed; errno=", getlasterror(), "\n")
...
func preemptM(mp *m) {
	...
	var thread uintptr
	if stdcall7(_DuplicateHandle, currentProcess, mp.thread, currentProcess, uintptr(unsafe.Pointer(&thread)), 0, 0, _DUPLICATE_SAME_ACCESS) == 0 {
		hidePrint("runtime.preemptM: duplicatehandle failed; errno=", getlasterror(), "\n")

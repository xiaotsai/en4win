...
func (p *pageAlloc) init(mheapLock *mutex, sysStat *sysMemStat, test bool) {
	if levelLogPages[0] > logMaxPackedValue {
		...
		hidePrint("runtime: root level max pages = ", 1<<levelLogPages[0], "\n")
		hidePrint("runtime: summary max pages = ", maxPackedValue, "\n")
...
func (p *pageAlloc) find(npages uintptr) (uintptr, offAddr) {
	...
	foundFree := func(addr offAddr, size uintptr) {
		if firstFree.base.lessEqual(addr) && addr.add(size-1).lessEqual(firstFree.bound) {
			...
			// so throw.
			hidePrint("runtime: addr = ", hex(addr.addr()), ", size = ", size, "\n")
			hidePrint("runtime: base = ", hex(firstFree.base.addr()), ", bound = ", hex(firstFree.bound.addr()), "\n")
...
nextLevel:
	for l := 0; l < len(p.summary); l++ {
		...
		hidePrint("runtime: summary[", l-1, "][", lastSumIdx, "] = ", lastSum.start(), ", ", lastSum.max(), ", ", lastSum.end(), "\n")
		hidePrint("runtime: level = ", l, ", npages = ", npages, ", j0 = ", j0, "\n")
		hidePrint("runtime: p.searchAddr = ", hex(p.searchAddr.addr()), ", i = ", i, "\n")
		hidePrint("runtime: levelShift[level] = ", levelShift[l], ", levelBits[level] = ", levelBits[l], "\n")
		for j := 0; j < len(entries); j++ {
			sum := entries[j]
			hidePrint("runtime: summary[", l, "][", i+j, "] = (", sum.start(), ", ", sum.max(), ", ", sum.end(), ")\n")
		}
	...
	if j == ^uint(0) {
		...
		hidePrint("runtime: summary[", len(p.summary)-1, "][", i, "] = (", sum.start(), ", ", sum.max(), ", ", sum.end(), ")\n")
		hidePrint("runtime: npages = ", npages, "\n")
		throw("bad summary data")
	}
...
func (p *pageAlloc) alloc(npages uintptr) (addr uintptr, scav uintptr) {
	...
	if pallocChunkPages-chunkPageIndex(p.searchAddr.addr()) >= uint(npages) {
		...
		if max := p.summary[len(p.summary)-1][i].max(); max >= uint(npages) {
			...
			if j == ^uint(0) {
				hidePrint("runtime: max = ", max, ", npages = ", npages, "\n")
				hidePrint("runtime: searchIdx = ", chunkPageIndex(p.searchAddr.addr()), ", p.searchAddr = ", hex(p.searchAddr.addr()), "\n")
				throw("bad summary data")
			}

...
func (s *mspan) divideByElemSize(n uintptr) uintptr {
	...
	if doubleCheck && q != n/s.elemsize {
		hidePrint(n, "/", s.elemsize, "should be", n/s.elemsize, "but got", q)
...
func badPointer(s *mspan, p, refBase, refOff uintptr) {
	...
	// in allocated spans.
	printlock()
	hidePrint("runtime: pointer ", hex(p))
	if s != nil {
		state := s.state.get()
		if state != mSpanInUse {
			hidePrint(" to unallocated span")
		} else {
			hidePrint(" to unused region of span")
		}
		hidePrint(" span.base()=", hex(s.base()), " span.limit=", hex(s.limit), " span.state=", state)
	}
	hidePrint("\n")
	if refBase != 0 {
		hidePrint("runtime: found in object at *(", hex(refBase), "+", hex(refOff), ")\n")
...
//
//go:linkname reflect_verifyNotInHeapPtr reflect.dvvw1_IpyIwu
...
func typeBitsBulkBarrier(typ *_type, dst, src, size uintptr) {
	...
	if typ.Size_ != size {
		hidePrint("runtime: typeBitsBulkBarrier with type ", toRType(typ).string(), " of size ", typ.Size_, " but memory size", size)
		throw("runtime: invalid typeBitsBulkBarrier")
	}
	if typ.Kind_&kindGCProg != 0 {
		hidePrint("runtime: typeBitsBulkBarrier with type ", toRType(typ).string(), " with GC prog")
		throw("runtime: invalid typeBitsBulkBarrier")
	}
	if !writeBarrier.needed {
		return
...
func dumpGCProg(p *byte) {
	...
	for {
		...
		if x&0x80 == 0 {
			...
			hidePrint("\t", nptr, " repeat ", nbit, " × ", count, "\n")
			nptr += nbit * count
		}
	}
}
...
//
//go:linkname reflect_gcbits reflect.dveCSYD

...
func winthrow(info *exceptionrecord, r *context, gp *g) {
	...
	hidePrint("Exception ", hex(info.exceptioncode), " ", hex(info.exceptioninformation[0]), " ", hex(info.exceptioninformation[1]), " ", hex(r.ip()), "\n")

	hidePrint("PC=", hex(r.ip()), "\n")
	if g0.m.incgo && gp == g0.m.g0 && g0.m.curg != nil {
		if iscgo {
			hidePrint("signal arrived during external code execution\n")
		}
		gp = g0.m.curg
	}
	hidePrint("\n")
...
func sigpanic() {
	...
	case _EXCEPTION_ACCESS_VIOLATION, _EXCEPTION_IN_PAGE_ERROR:
		...
		if inUserArenaChunk(gp.sigcode1) {
			...
			// that it is.
			hidePrint("accessed data from freed user arena ", hex(gp.sigcode1), "\n")
		} else {
			hidePrint("unexpected fault address ", hex(gp.sigcode1), "\n")
		}
		throw("fault")
	case _EXCEPTION_INT_DIVIDE_BY_ZERO:
		panicdivide()

...
func sysUnusedOS(v unsafe.Pointer, n uintptr) {
	...
	for n > 0 {
		...
		if small < 4096 {
			hidePrint("runtime: VirtualFree of ", small, " bytes failed with errno=", getlasterror(), "\n")
	...
	for k > 0 {
		...
		if small < 4096 {
			...
			case _ERROR_NOT_ENOUGH_MEMORY, _ERROR_COMMITMENT_LIMIT:
				hidePrint("runtime: VirtualAlloc of ", n, " bytes failed with errno=", errno, "\n")
				throw("out of memory")
			default:
				hidePrint("runtime: VirtualAlloc of ", small, " bytes failed with errno=", errno, "\n")
...
func sysFreeOS(v unsafe.Pointer, n uintptr) {
	...
	if r == 0 {
		hidePrint("runtime: VirtualFree of ", n, " bytes failed with errno=", getlasterror(), "\n")

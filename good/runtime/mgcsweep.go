	...
	for {
		...
		if a.state.CompareAndSwap(state, state-1) {
			...
			if debug.gcpacertrace > 0 {
				...
				hidePrint("pacer: sweep done at heap size ", live>>20, "MB; allocated ", (live-mheap_.sweepHeapLiveBasis)>>20, "MB during sweep; swept ", mheap_.pagesSwept.Load(), " pages at ", mheap_.sweepPagesPerByte, " pages/byte\n")
			}
			return
		}
	}
	...
	for {
		...
		if state := s.state.get(); state != mSpanInUse {
			...
			if !(s.sweepgen == sl.sweepGen || s.sweepgen == sl.sweepGen+3) {
				hidePrint("runtime: bad span s.state=", state, " s.sweepgen=", s.sweepgen, " sweepgen=", sl.sweepGen, "\n")
...
func (sl *sweepLocked) sweep(preserve bool) bool {
	...
	if state := s.state.get(); state != mSpanInUse || s.sweepgen != sweepgen-1 {
		hidePrint("mspan.sweep: state=", state, " sweepgen=", s.sweepgen, " mheap.sweepgen=", sweepgen, "\n")
	...
	if nalloc > s.allocCount {
		...
		// more detail.
		hidePrint("runtime: nelems=", s.nelems, " nalloc=", nalloc, " previous allocCount=", s.allocCount, " nfreed=", nfreed, "\n")
...
func (s *mspan) reportZombies() {
	printlock()
	hidePrint("runtime: marked free object in span ", s, ", elemsize=", s.elemsize, " freeindex=", s.freeindex, " (bad use of unsafe.Pointer? try -d=checkptr)\n")
	mbits := s.markBitsForBase()
	abits := s.allocBitsForIndex(0)
	for i := uintptr(0); i < s.nelems; i++ {

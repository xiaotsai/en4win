...
func (r *debugLogReader) printVal() bool {
	...
	switch typ {
	default:
		hidePrint("<unknown field type ", hex(typ), " pos ", r.begin-1, " end ", r.end, ">\n")
		return false

	case debugLogUnknown:
		hidePrint("<unknown kind>")
	...
	case debugLogHex, debugLogPtr:
		hidePrint(hex(r.uvarint()))

	case debugLogString:
		sl := r.uvarint()
		if r.begin+sl > r.end {
			r.begin = r.end
			hidePrint("<string length corrupted>")
			break
		}
		for sl > 0 {
			b := r.data.b[r.begin%uint64(len(r.data.b)):]
	...
	case debugLogStringOverflow:
		hidePrint("..(", r.uvarint(), " more bytes)..")
...
func printDebugLog() {
	...
	if state1 == nil {
		hidePrint("failed to allocate read state for", n, "logs")
		printunlock()
		return
	}
	...
	for {
		...
		s := &state[best.i]
		if s.first {
			hidePrint(">> begin log ", best.i)
			if s.lost != 0 {
				hidePrint("; lost first ", s.lost>>10, "KB")
			}
			hidePrint(" <<\n")
			s.first = false
		}
		...
		hidePrint(slicebytetostringtmp((*byte)(noescape(unsafe.Pointer(&pnanoBytes[0]))), len(pnanoBytes)))
		hidePrint(" P ", p, "] ")
...
func printDebugLogPC(pc uintptr, returnPC bool) {
	...
	hidePrint(hex(pc))
	if !fn.valid() {
		hidePrint(" [unknown PC]")
	} else {
		name := funcname(fn)
		file, line := funcline(fn, pc)
		hidePrint(" [", name, "+", hex(pc-fn.entry()),
			" ", file, ":", line, "]")
	}
}

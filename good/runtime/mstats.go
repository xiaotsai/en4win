...
func readmemstats_m(stats *MemStats) {
	...
	if gcController.heapInUse.load() != uint64(consStats.inHeap) {
		hidePrint("runtime: heapInUse=", gcController.heapInUse.load(), "\n")
		hidePrint("runtime: consistent value=", consStats.inHeap, "\n")
		throw("heapInUse and consistent stats are not equal")
	}
	if gcController.heapReleased.load() != uint64(consStats.released) {
		hidePrint("runtime: heapReleased=", gcController.heapReleased.load(), "\n")
		hidePrint("runtime: consistent value=", consStats.released, "\n")
	...
	if heapRetained != consRetained {
		hidePrint("runtime: global value=", heapRetained, "\n")
		hidePrint("runtime: consistent value=", consRetained, "\n")
		throw("measures of the retained heap are not equal")
	}
	if gcController.totalAlloc.Load() != totalAlloc {
		hidePrint("runtime: totalAlloc=", gcController.totalAlloc.Load(), "\n")
		hidePrint("runtime: consistent value=", totalAlloc, "\n")
		throw("totalAlloc and consistent stats are not equal")
	}
	if gcController.totalFree.Load() != totalFree {
		hidePrint("runtime: totalFree=", gcController.totalFree.Load(), "\n")
		hidePrint("runtime: consistent value=", totalFree, "\n")
	...
	if gcController.mappedReady.Load() != totalMapped-uint64(consStats.released) {
		hidePrint("runtime: mappedReady=", gcController.mappedReady.Load(), "\n")
		hidePrint("runtime: totalMapped=", totalMapped, "\n")
		hidePrint("runtime: released=", uint64(consStats.released), "\n")
		hidePrint("runtime: totalMapped-released=", totalMapped-uint64(consStats.released), "\n")
...
//go:linkname readGCStats G1I6Ii.lmRUxT

...
func gopanic(e any) {
	...
	if gp.m.preemptoff != "" {
		...
		printany(e)
		hidePrint("\n")
		hidePrint("preempt off reason: ")
		hidePrint(gp.m.preemptoff)
		hidePrint("\n")
...
//go:linkname sync_throw sync.f6Asug
func sync_throw(s string) {
	throw(s)
}

//go:linkname sync_fatal sync.h7pm4f2ZL
...
func recovery(gp *g) {
	...
	if sp != 0 && (sp < gp.stack.lo || gp.stack.hi < sp) {
		hidePrint("recover: ", hex(sp), " not in [", hex(gp.stack.lo), ", ", hex(gp.stack.hi), "]\n")
		throw("bad recovery")
	}
...
func startpanic_m() bool {
	...
	if mheap_.cachealloc.size == 0 {	// very early
		hidePrint("runtime: panic before malloc heap initialized\n")
	}
	...
	case 1:
		...
		gp.m.dying = 2
		hidePrint("panic during panic\n")
		return false
	case 2:
		// This is a genuine bug in the runtime, we couldn't even
		// print the stack trace successfully.
		gp.m.dying = 3
		hidePrint("stack trace unavailable\n")
		exit(4)
		fallthrough
	default:
...
func dopanic_m(gp *g, pc, sp uintptr) bool {
	if gp.sig != 0 {
		...
		if signame != "" {
			hidePrint("[signal ", signame)
		} else {
			hidePrint("[signal ", hex(gp.sig))
		}
		hidePrint(" code=", hex(gp.sigcode0), " addr=", hex(gp.sigcode1), " pc=", hex(gp.sigpc), "]\n")
	}

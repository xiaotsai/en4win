...
func mallocinit() {
	...
	if physPageSize > maxPhysPageSize {
		hidePrint("system page size (", physPageSize, ") is larger than maximum page size (", maxPhysPageSize, ")\n")
		throw("bad system page size")
	}
	if physPageSize < minPhysPageSize {
		hidePrint("system page size (", physPageSize, ") is smaller than minimum page size (", minPhysPageSize, ")\n")
		throw("bad system page size")
	}
	if physPageSize&(physPageSize-1) != 0 {
		hidePrint("system page size (", physPageSize, ") must be a power of 2\n")
		throw("bad system page size")
	}
	if physHugePageSize&(physHugePageSize-1) != 0 {
		hidePrint("system huge page size (", physHugePageSize, ") must be a power of 2\n")
	...
	if pagesPerArena%pagesPerSpanRoot != 0 {
		hidePrint("pagesPerArena (", pagesPerArena, ") is not divisible by pagesPerSpanRoot (", pagesPerSpanRoot, ")\n")
		throw("bad pagesPerSpanRoot")
	}
	if pagesPerArena%pagesPerReclaimerChunk != 0 {
		hidePrint("pagesPerArena (", pagesPerArena, ") is not divisible by pagesPerReclaimerChunk (", pagesPerReclaimerChunk, ")\n")
...
func (h *mheap) sysAlloc(n uintptr, hintList **arenaHint, register bool) (v unsafe.Pointer, size uintptr) {
	...
	// Check for bad pointers or pointers we can't use.
	{
		...
		if bad != "" {
			...
			hidePrint("runtime: memory allocated by OS [", hex(p), ", ", hex(p+size), ") not in usable address space: ", bad, "\n")
...
func (c *mcache) nextFree(spc spanClass) (v gclinkptr, s *mspan, shouldhelpgc bool) {
	...
	if freeIndex == s.nelems {
		...
		if uintptr(s.allocCount) != s.nelems {
			hidePrint("runtime: s.allocCount=", s.allocCount, "s.nelems=", s.nelems)
	...
	s.allocCount++
	if uintptr(s.allocCount) > s.nelems {
		hidePrint("s.allocCount=", s.allocCount, "s.nelems=", s.nelems)
...
//go:linkname reflect_unsafe_New reflect.qscv_5
func reflect_unsafe_New(typ *_type) unsafe.Pointer {
	return mallocgc(typ.Size_, typ, true)
}

//go:linkname reflectlite_unsafe_New yAj8QghzkR.cUo1caLrG
...
//go:linkname reflect_unsafe_NewArray reflect.l9WZQcaw

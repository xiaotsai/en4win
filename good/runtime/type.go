...
func resolveNameOff(ptrInModule unsafe.Pointer, off nameOff) name {
	...
	for md := &firstmoduledata; md != nil; md = md.next {
		if base >= md.types && base < md.etypes {
			...
			if res > md.etypes {
				hidePrint("runtime: nameOff", hex(off), "out of range", hex(md.types), "-", hex(md.etypes))
	...
	reflectOffsUnlock()
	if !found {
		hidePrint("runtime: nameOff", hex(off), "base", hex(base), "not in ranges:")
		for next := &firstmoduledata; next != nil; next = next.next {
			hidePrint("\ttypes", hex(next.types), "etypes", hex(next.etypes))
		}
...
func resolveTypeOff(ptrInModule unsafe.Pointer, off typeOff) *_type {
	...
	if md == nil {
		...
		reflectOffsUnlock()
		if res == nil {
			hidePrint("runtime: typeOff", hex(off), "base", hex(base), "not in ranges:")
			for next := &firstmoduledata; next != nil; next = next.next {
				hidePrint("\ttypes", hex(next.types), "etypes", hex(next.etypes))
			}
	...
	res := md.types + uintptr(off)
	if res > md.etypes {
		hidePrint("runtime: typeOff", hex(off), "out of range", hex(md.types), "-", hex(md.etypes))
...
func (t rtype) textOff(off textOff) unsafe.Pointer {
	...
	if md == nil {
		...
		reflectOffsUnlock()
		if res == nil {
			hidePrint("runtime: textOff", hex(off), "base", hex(base), "not in ranges:")
			for next := &firstmoduledata; next != nil; next = next.next {
				hidePrint("\ttypes", hex(next.types), "etypes", hex(next.etypes))
			}
...
func typesEqual(t, v *_type, seen map[_typePair]struct{}) bool {
	...
	case kindStruct:
		...
		return true
	default:
		hidePrint("runtime: impossible type kind", kind)

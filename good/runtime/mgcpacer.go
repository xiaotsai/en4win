...
func (c *gcControllerState) startCycle(markStartTime int64, procs int, trigger gcTrigger) {
	...
	if debug.gcpacertrace > 0 {
		...
		hidePrint("pacer: assist ratio=", assistRatio,
...
func (c *gcControllerState) endCycle(now int64, procs int, userForced bool) {
	...
	if debug.gcpacertrace > 0 {
		...
		hidePrint("pacer: ", int(utilization*100), "% CPU (", int(goal), " exp.) for ")
		hidePrint(c.heapScanWork.Load(), "+", c.stackScanWork.Load(), "+", c.globalsScanWork.Load(), " B work (", c.lastHeapScan+c.lastStackScan.Load()+c.globalsScan.Load(), " B exp.) ")
		live := c.heapLive.Load()
		hidePrint("in ", c.triggered, " B -> ", live, " B (∆goal ", int64(live)-int64(c.lastHeapGoal), ", cons/mark ", oldConsMark, ")")
		hidePrint()
		printunlock()
	}
}
...
func (c *gcControllerState) trigger() (uint64, uint64) {
	...
	if trigger > goal {
		hidePrint("trigger=", trigger, " heapGoal=", goal, "\n")
		hidePrint("minTrigger=", minTrigger, " maxTrigger=", maxTrigger, "\n")
...
//go:linkname setGCPercent G1I6Ii.nvFQvkZmVJo
...
//go:linkname setMemoryLimit G1I6Ii.atoZR8e7aQGj

...
func gcMarkRootCheck() {
	if work.markrootNext < work.markrootJobs {
		hidePrint(work.markrootNext, " of ", work.markrootJobs, " markroot jobs done\n")
...
func markroot(gcw *gcWork, i uint32, flushBgCredit bool) int64 {
	...
	default:
		...
		if i < work.baseStacks || work.baseEnd <= i {
			printlock()
			hidePrint("runtime: markroot index ", i, " not in stack roots range [", work.baseStacks, ", ", work.baseEnd, ")\n")
...
func gcAssistAlloc1(gp *g, scanWork int64) {
	...
	if decnwait == work.nproc {
		hidePrint("runtime: work.nwait =", decnwait, "work.nproc=", work.nproc)
	...
	if incnwait > work.nproc {
		hidePrint("runtime: work.nwait=", incnwait,
...
func scanstack(gp *g, gcw *gcWork) int64 {
	if readgstatus(gp)&_Gscan == 0 {
		hidePrint("runtime:scanstack: gp=", gp, ", goid=", gp.goid, ", gp->atomicstatus=", hex(readgstatus(gp)), "\n")
		throw("scanstack - bad status")
	}

	switch readgstatus(gp) &^ _Gscan {
	default:
		hidePrint("runtime: gp=", gp, ", goid=", gp.goid, ", gp->atomicstatus=", readgstatus(gp), "\n")
	...
	if stackTraceDebug {
		hidePrint("stack trace goroutine", gp.goid)
	}

	if debugScanConservative && gp.asyncSafePoint {
		hidePrint("scanning async preempted goroutine ", gp.goid, " stack [", hex(gp.stack.lo), ",", hex(gp.stack.hi), ")\n")
	}
	...
	for {
		...
		if stackTraceDebug {
			printlock()
			hidePrint("  live stkobj at", hex(state.stack.lo+uintptr(obj.off)), "of size", obj.size)
			if conservative {
				hidePrint(" (conservative)")
			}
			hidePrint()
			printunlock()
		}
	...
	for state.head != nil {
		...
		if stackTraceDebug {
			for i := 0; i < x.nobj; i++ {
				...
				hidePrint("  dead stkobj at", hex(gp.stack.lo+uintptr(obj.off)), "of size", obj.r.size)
...
func scanframeworker(frame *stkframe, state *stackScanState, gcw *gcWork) {
	if _DebugGC > 1 && frame.continpc != 0 {
		hidePrint("scanframe ", funcname(frame.fn), "\n")
	}
	...
	if state.conservative || isAsyncPreempt || isDebugCall {
		if debugScanConservative {
			hidePrint("conservatively scanning function", funcname(frame.fn), "at PC", hex(frame.continpc))
		}
	...
	if frame.varp != 0 {
		...
		for i := range objs {
			...
			if stackTraceDebug {
				hidePrint("stkobj at", hex(ptr), "of size", obj.size)
			}
...
func scanConservative(b, n uintptr, ptrmask *uint8, gcw *gcWork, state *stackScanState) {
	if debugScanConservative {
		printlock()
		hidePrint("conservatively scanning [", hex(b), ",", hex(b+n), ")\n")
...
func greyobject(obj, base, off uintptr, span *mspan, gcw *gcWork, objIndex uintptr) {
	...
	if useCheckmark {
		...
		if debug.gccheckmark > 0 && span.isFree(objIndex) {
			hidePrint("runtime: marking free object ", hex(obj), " found at *(", hex(base), "+", hex(off), ")\n")
...
func gcDumpObject(label string, obj, off uintptr) {
	s := spanOf(obj)
	hidePrint(label, "=", hex(obj))
	if s == nil {
		hidePrint(" s=nil\n")
		return
	}
	hidePrint(" s.base()=", hex(s.base()), " s.limit=", hex(s.limit), " s.spanclass=", s.spanclass, " s.elemsize=", s.elemsize, " s.state=")
	if state := s.state.get(); 0 <= state && int(state) < len(mSpanStateNames) {
		hidePrint(mSpanStateNames[state], "\n")
	} else {
		hidePrint("unknown(", state, ")\n")
	}
	...
	for i := uintptr(0); i < size; i += goarch.PtrSize {
		...
		hidePrint(" *(", label, "+", i, ") = ", hex(*(*uintptr)(unsafe.Pointer(obj + i))))
		if i == off {
			hidePrint(" <==")
		}
		hidePrint("\n")

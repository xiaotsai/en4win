...
//
//go:linkname runtime_FrameStartLine l4ycaydqJMa.fZ6uE5
...
// the shape name to "...".
//
//go:linkname runtime_FrameSymbolName l4ycaydqJMa.x9EnC3
...
//
//go:linkname runtime_expandFinalInlineFrame l4ycaydqJMa.pwOWLuk
...
func moduledataverify1(datap *moduledata) {
	...
	hdr := datap.pcHeader
	if hdr.magic != 3121825982 || hdr.pad1 != 0 || hdr.pad2 != 0 ||
		hdr.minLC != sys.PCQuantum || hdr.ptrSize != goarch.PtrSize || hdr.textStart != datap.text {
		hidePrint("runtime: pcHeader: magic=", hex(hdr.magic), "pad1=", hdr.pad1, "pad2=", hdr.pad2,
	...
	for i := 0; i < nftab; i++ {
		...
		if datap.ftab[i].entryoff > datap.ftab[i+1].entryoff {
			...
			hidePrint("function symbol table not sorted by PC offset:", hex(datap.ftab[i].entryoff), funcname(f1), ">", hex(datap.ftab[i+1].entryoff), f2name, ", plugin:", datap.pluginpath)
			for j := 0; j <= i; j++ {
				hidePrint("\t", hex(datap.ftab[j].entryoff), funcname(funcInfo{(*_func)(unsafe.Pointer(&datap.pclntable[datap.ftab[j].funcoff])), datap}))
			}
			if GOOS == "aix" && isarchive {
				hidePrint("-Wl,-bnoobjreorder is mandatory on aix/ppc64 with c-archive")
			}
	...
	if datap.minpc != min || datap.maxpc != max {
		hidePrint("minpc=", hex(datap.minpc), "min=", hex(min), "maxpc=", hex(datap.maxpc), "max=", hex(max))
		throw("minpc or maxpc invalid")
	}

	for _, modulehash := range datap.modulehashes {
		if modulehash.linktimehash != *modulehash.runtimehash {
			hidePrint("abi mismatch detected between", datap.modulename, "and", modulehash.modulename)
			throw("abi mismatch")
		}
	}
}
...
func (md *moduledata) textAddr(off32 uint32) uintptr {
	...
	if len(md.textsectmap) > 1 {
		...
		if res > md.etext && GOARCH != "wasm" {	// on wasm, functions do not live in the same address space as the linear memory
			hidePrint("runtime: textAddr", hex(res), "out of range", hex(md.text), "-", hex(md.etext))
...
func (f funcInfo) entry() uintptr {
	return f.datap.textAddr(f.entryOff ^ (uint32(f.nameOff) * 349165703))
}
...
func pcvalue(f funcInfo, off uint32, targetpc uintptr, cache *pcvalueCache, strict bool) (int32, uintptr) {
	...
	if !f.valid() {
		if strict && panicking.Load() == 0 {
			hidePrint("runtime: no module data for", hex(f.entry()))
	...
	hidePrint("runtime: invalid pc-encoded table f=", funcname(f), " pc=", hex(pc), " targetpc=", hex(targetpc), " tab=", p, "\n")
	...
	for {
		...
		hidePrint("\tvalue=", val, " until pc=", hex(pc), "\n")
	}
...
func funcspdelta(f funcInfo, targetpc uintptr, cache *pcvalueCache) int32 {
	...
	if debugPcln && x&(goarch.PtrSize-1) != 0 {
		hidePrint("invalid spdelta ", funcname(f), " ", hex(f.entry()), " ", hex(targetpc), " ", hex(f.pcsp), " ", x, "\n")
		throw("bad spdelta")
	}
	return x
}

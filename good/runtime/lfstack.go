...
func (head *lfstack) push(node *lfnode) {
	...
	if node1 := lfstackUnpack(new); node1 != node {
		hidePrint("runtime: lfstack.push invalid packing: node=", node, " cnt=", hex(node.pushcnt), " packed=", hex(new), " -> node=", node1, "\n")
		throw("lfstack.push")
	}
	for {
...
func lfnodeValidate(node *lfnode) {
	...
	if lfstackUnpack(lfstackPack(node, ^uintptr(0))) != node {
		printlock()
		hidePrint("runtime: bad lfnode address", hex(uintptr(unsafe.Pointer(node))))

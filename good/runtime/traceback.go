...
func (u *unwinder) initAt(pc0, sp0, lr0 uintptr, gp *g, flags unwindFlags) {
	...
	if !f.valid() {
		if flags&unwindSilentErrors == 0 {
			hidePrint("runtime: g ", gp.goid, ": unknown pc ", hex(frame.pc), "\n")
...
func (u *unwinder) resolveInternal(innermost, isSyscall bool) {
	...
	if flag&abi.FuncFlagTopFrame != 0 {
		...
		if u.flags&unwindSilentErrors == 0 && !innermost {
			hidePrint("traceback: unexpected SPWRITE function", funcname(f))
...
func (u *unwinder) next() {
	...
	if !flr.valid() {
		...
		if fail || doPrint {
			hidePrint("runtime: g ", gp.goid, ": unexpected return pc for ", funcname(f), " called from ", hex(frame.lr), "\n")
			tracebackHexdump(gp.stack, frame, 0)
		}
		if fail {
	...
	if frame.pc == frame.lr && frame.sp == frame.fp {
		...
		hidePrint("runtime: traceback stuck. pc=", hex(frame.pc), " sp=", hex(frame.sp), "\n")
...
func (u *unwinder) finishInternal() {
	...
	if u.flags&(unwindPrintErrors|unwindSilentErrors) == 0 && u.frame.sp != gp.stktopsp {
		hidePrint("runtime: g", gp.goid, ": frame.sp=", hex(u.frame.sp), " top=", hex(gp.stktopsp), "\n")
		hidePrint("\tstack=[", hex(gp.stack.lo), "-", hex(gp.stack.hi), "\n")
...
func printArgs(f funcInfo, argp unsafe.Pointer, pc uintptr) {
	...
	// max length of _FUNCDATA_ArgInfo (see the compiler side for reasoning)
		...
		for iu, uf := newInlineUnwinder(f, u.symPC(), noEscapePtr(&u.cache)); uf.valid(); uf = iu.next(uf) {
			...
			if !iu.isInlined(uf) {
				if u.frame.pc > f.entry() {
					hidePrint(" +", hex(u.frame.pc-f.entry()))
				}
				if gp.m != nil && gp.m.throwing >= throwTypeRuntime && gp == gp.m.curg || level >= 2 {
					hidePrint(" fp=", hex(u.frame.fp), " sp=", hex(u.frame.sp), " pc=", hex(u.frame.pc))
				}
			}
			hidePrint("\n")
		}
			...
			for _, pc := range cgoBuf[:cgoN] {
				if cgoSymbolizer == nil {
					if pr, stop := commitFrame(); stop {
						...
						hidePrint("non-Go function at pc=", hex(pc), "\n")
					}
				} else {

...
func gcStart(trigger gcTrigger) {
	...
	for _, p := range allp {
		if fg := p.mcache.flushGen.Load(); fg != mheap_.sweepgen {
			hidePrint("runtime: p", p.id, "flushGen", fg, "!= sweepgen", mheap_.sweepgen)
...
func gcMarkTermination() {
	...
	if debug.gctrace > 0 {
		...
		for i, ns := range []int64{work.tMark, work.tMarkTerm, work.tEnd} {
			...
			hidePrint(string(fmtNSAsMS(sbuf[:], uint64(ns-prev))))
			prev = ns
		}
		hidePrint(" ms clock, ")
		for i, ns := range []int64{
			...
			hidePrint(string(fmtNSAsMS(sbuf[:], uint64(ns))))
		}
		hidePrint(" ms cpu, ",
...
func gcBgMarkWorker() {
	...
	for {
		...
		if gcBlackenEnabled == 0 {
			hidePrint("worker mode", pp.gcMarkWorkerMode)
		...
		if incnwait > work.nproc {
			hidePrint("runtime: p.gcMarkWorkerMode=", pp.gcMarkWorkerMode,
...
func gcMark(startTime int64) {
	...
	if work.full != 0 || work.markrootNext < work.markrootJobs {
		hidePrint("runtime: full=", hex(work.full), " next=", work.markrootNext, " jobs=", work.markrootJobs, " nDataRoots=", work.nDataRoots, " nBSSRoots=", work.nBSSRoots, " nSpanRoots=", work.nSpanRoots, " nStackRoots=", work.nStackRoots, "\n")
	...
	for _, p := range allp {
		...
		if !gcw.empty() {
			printlock()
			hidePrint("runtime: P ", p.id, " flushedWork ", gcw.flushedWork)
			if gcw.wbuf1 == nil {
				hidePrint(" wbuf1=<nil>")
			} else {
				hidePrint(" wbuf1.n=", gcw.wbuf1.nobj)
			}
			if gcw.wbuf2 == nil {
				hidePrint(" wbuf2=<nil>")
			} else {
				hidePrint(" wbuf2.n=", gcw.wbuf2.nobj)
			}
			hidePrint("\n")
...
//go:linkname sync_runtime_registerPoolCleanup sync.ew66MMK
func sync_runtime_registerPoolCleanup(f func()) {
	poolcleanup = f
}

//go:linkname boring_registerCache cx3VtVo.arR0Dnwm
...
func gcTestIsReachable(ptrs ...unsafe.Pointer) (mask uint64) {
	...
	for i, s := range specials {
		if !s.done {
			printlock()
			hidePrint("runtime: object", i, "was not swept")

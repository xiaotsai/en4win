...
func stackcacherefill(c *mcache, order uint8) {
	if stackDebug >= 1 {
		hidePrint("stackcacherefill order=", order, "\n")
	}
...
func stackcacherelease(c *mcache, order uint8) {
	if stackDebug >= 1 {
		hidePrint("stackcacherelease order=", order, "\n")
	}
...
func adjustpointer(adjinfo *adjustinfo, vpp unsafe.Pointer) {
	...
	if adjinfo.old.lo <= p && p < adjinfo.old.hi {
		*pp = p + adjinfo.delta
		if stackDebug >= 3 {
			hidePrint("        adjust ptr ", pp, ":", hex(p), " -> ", hex(*pp), "\n")
		}
	}
}
	...
	for i := uintptr(0); i < num; i += 8 {
		if stackDebug >= 4 {
			for j := uintptr(0); j < 8; j++ {
				hidePrint("        ", add(scanp, (i+j)*goarch.PtrSize), ":", ptrnames[bv.ptrbit(i+j)], ":", hex(*(*uintptr)(add(scanp, (i+j)*goarch.PtrSize))), " # ", i, " ", *addb(bv.bytedata, i/8), "\n")
			}
		}
		b := *(addb(bv.bytedata, i/8))
		for b != 0 {
		...
		retry:
			...
			if f.valid() && 0 < p && p < minLegalPointer && debug.invalidptr != 0 {
				...
				// Live analysis wrong?
				getg().m.traceback = 2
				hidePrint("runtime: bad pointer in frame ", funcname(f), " at ", pp, ": ", hex(p), "\n")
				throw("invalid pointer found on stack")
			}
			if minp <= p && p < maxp {
				if stackDebug >= 3 {
					hidePrint("adjust ptr ", hex(p), " ", funcname(f), "\n")
				}
				if useCAS {
...
func adjustframe(frame *stkframe, adjinfo *adjustinfo) {
	...
	f := frame.fn
	if stackDebug >= 2 {
		hidePrint("    adjusting ", funcname(f), " frame=[", hex(frame.sp), ",", hex(frame.fp), "] pc=", hex(frame.pc), " continpc=", hex(frame.continpc), "\n")
	}
	...
	if (goarch.ArchFamily == goarch.AMD64 || goarch.ArchFamily == goarch.ARM64) && frame.argp-frame.varp == 2*goarch.PtrSize {
		...
		if debugCheckBP {
			...
			if bp != 0 && (bp < adjinfo.old.lo || bp >= adjinfo.old.hi) {
				hidePrint("runtime: found invalid frame pointer")
				hidePrint("bp=", hex(bp), " min=", hex(adjinfo.old.lo), " max=", hex(adjinfo.old.hi), "\n")
...
func adjustctxt(gp *g, adjinfo *adjustinfo) {
	...
	if debugCheckBP {
		bp := gp.sched.bp
		if bp != 0 && (bp < adjinfo.old.lo || bp >= adjinfo.old.hi) {
			hidePrint("runtime: found invalid top frame pointer")
			hidePrint("bp=", hex(bp), " min=", hex(adjinfo.old.lo), " max=", hex(adjinfo.old.hi), "\n")
...
func copystack(gp *g, newsize uintptr) {
	...
	if stackDebug >= 1 {
		hidePrint("copystack gp=", gp, " [", hex(old.lo), " ", hex(old.hi-used), " ", hex(old.hi), "]", " -> [", hex(new.lo), " ", hex(new.hi-used), " ", hex(new.hi), "]/", newsize, "\n")
	}
...
func newstack() {
	...
	if thisg.m.morebuf.g.ptr() != thisg.m.curg {
		hidePrint("runtime: newstack called from g=", hex(thisg.m.morebuf.g), "\n"+"\tm=", thisg.m, " m->curg=", thisg.m.curg, " m->g0=", thisg.m.g0, " m->gsignal=", thisg.m.gsignal, "\n")
	...
	if thisg.m.curg.throwsplit {
		...
		hidePrint("runtime: newstack at ", pcname, "+", hex(pcoff),
	...
	if stackDebug >= 1 || sp < gp.stack.lo {
		hidePrint("runtime: newstack sp=", hex(sp), " stack=[", hex(gp.stack.lo), ", ", hex(gp.stack.hi), "]\n",
			"\tmorebuf={pc:", hex(morebuf.pc), " sp:", hex(morebuf.sp), " lr:", hex(morebuf.lr), "}\n",
			"\tsched={pc:", hex(gp.sched.pc), " sp:", hex(gp.sched.sp), " lr:", hex(gp.sched.lr), " ctxt:", gp.sched.ctxt, "}\n")
	}
	if sp < gp.stack.lo {
		hidePrint("runtime: gp=", gp, ", goid=", gp.goid, ", gp->status=", hex(readgstatus(gp)), "\n ")
		hidePrint("runtime: split stack overflow: ", hex(sp), " < ", hex(gp.stack.lo), "\n")
	...
	if newsize > maxstacksize || newsize > maxstackceiling {
		if maxstacksize < maxstackceiling {
			hidePrint("runtime: goroutine stack exceeds ", maxstacksize, "-byte limit\n")
		} else {
			hidePrint("runtime: goroutine stack exceeds ", maxstackceiling, "-byte limit\n")
		}
		hidePrint("runtime: sp=", hex(sp), " stack=[", hex(gp.stack.lo), ", ", hex(gp.stack.hi), "]\n")
...
func shrinkstack(gp *g) {
	...
	if stackDebug > 0 {
		hidePrint("shrinking stack ", oldsize, "->", newsize, "\n")
	}
